...
const detectMilestone = async (event, eventType) => {
  try {
    ...
    const contributorRef = db.collection('contributor_stats')
      .doc(`${repoName}_${contributor}`);
    
    const contributorDoc = await contributorRef.get();
    let currentStats = contributorDoc.exists ? contributorDoc.data() : {
      pull_requests: 0,
      stars: 0,
      issues: 0,
      commits: 0,
    ...
    switch (eventType) {
      case 'pull_request':
        if (event.action === 'closed' && event.pull_request?.merged) {
          ...
          milestone = checkMilestone('pull_request', currentStats.pull_requests, repoName, contributor);
        }
        break;
      ...
      case 'star':
        if (event.action === 'created') {
          ...
          milestone = checkMilestone('star', currentStats.stars, repoName, contributor);
        }
        break;
      ...
      case 'issues':
        if (event.action === 'opened') {
          ...
          milestone = checkMilestone('issue', currentStats.issues, repoName, contributor);
        }
        break;
        
      case 'push':
        if (event.commits && event.commits.length > 0) {
          currentStats.commits = (currentStats.commits || 0) + event.commits.length;
          milestone = checkMilestone('commit', currentStats.commits, repoName, contributor);
        }
        break;
    }
    ...
    if (eventType === 'push' || eventType === 'pull_request') {
      ...
      if (!milestone) {
        ...
        milestone = checkMilestone('contribution_days', contributionCount, repoName, contributor);
      }
    }
...
const checkMilestone = (eventType, count, repoName, contributor) => {
